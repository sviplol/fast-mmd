...
path = os.path.join('src', 'views', 'MainApp.vue')

content = r'''<template>
  <div class="main">
    <div class="mheader">
      <h2>{{ serverPlatform==='tk' ? 'TK Token' : 'GLM API' }}</h2>
...
.qr-box { background:#fff; border-radius:16px; padding:24px; text-align:center; width:320px; }
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print(f'MainApp.vue rewritten: {len(content)} chars')
